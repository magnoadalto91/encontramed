...
export default function FinanceiroScreen() {
  ...
  const [resumo, setResumo] = useState({ totalPago: 0, totalPendente: 0, totalCancelado: 0 });
  ...
  const carregar = useCallback(async (isRefresh = false) => {
    ...
    try {
      const data = await financeiroService.getExtrato();
    ...
    <LinearGradient colors={['#0A1628', '#0D1B2E']} style={{ flex: 1 }}>
      <SafeAreaView style={{ flex: 1 }}>
        ...
        <View style={styles.summaryRow}>
          <SummaryCard label="Recebido" value={resumo.totalPago} color={COLORS.success} />
          <SummaryCard label="A receber" value={resumo.totalPendente} color={COLORS.accent} />
        </View>
        ...
        <FlatList
          ...
          ListHeaderComponent={<Text style={styles.sectionTitle}>Histórico de transações</Text>}
          renderItem={({ item }) => (
            <Card style={styles.card}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                <Text style={styles.valor}>{formatCurrency(item.valorMedico || item.valor)}</Text>
              ...
              {item.valorTaxa > 0 && (
                <Text style={styles.taxa}>Taxa plataforma: {formatCurrency(item.valorTaxa)}</Text>
              )}
            </Card>
          )}
          ListEmptyComponent={
...
const styles = StyleSheet.create({
  ...
  summaryCard: {
    flex: 1, backgroundColor: '#112240', borderRadius: 12, padding: 16,
  ...
  plantaoTitulo: { color: COLORS.textSecondary, fontSize: 13, marginBottom: 4 },
  data: { color: COLORS.textMuted, fontSize: 12 },
  taxa: { color: COLORS.textMuted, fontSize: 11, marginTop: 4 },
